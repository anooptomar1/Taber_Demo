...
open class ImageCropperView: UIView, UIScrollViewDelegate, UIGestureRecognizerDelegate, ImageCropperOverlayViewDelegate {
    ...
    fileprivate struct SavedProperty {
        ...
        mutating func save(scrollView: ImageCropperScrollView) {
            ...
            let contentSize = CGSize(
                width   : (scrollView.contentSize.width - scrollView.visibleRectangle.width).cropperRoundTo(precision: 3),
                height  : (scrollView.contentSize.height - scrollView.visibleRectangle.height).cropperRoundTo(precision: 3))
            
            contentOffsetPercentage = CGPointPercentage(
                x: (contentOffset.x > 0 && contentSize.width != 0)
                    ? cropperRound(x: contentOffset.x / contentSize.width, multiplier: 0.005)
                    : 0,
                y: (contentOffset.y > 0 && contentSize.height != 0)
                    ? cropperRound(x: contentOffset.y / contentSize.height, multiplier: 0.005)
                    : 0)
            
            cropRectSize = scrollView.visibleRectangle.size
        }
    }
   ...
   // Saved Scroll View parameters before complex layout animation
    ...
    // The image displayed in the image cropper view. Default is - no image.
    ...
    // Cropped image in the specified crop rectangle
    
    open var croppedImage: UIImage? {
        return image?.ic_imageInRect(scrollView.scaledVisibleRectangle)?.ic_rotateByAngle(angle)
    }
    ...
    var scrollView: ImageCropperScrollView!
    
    // MARK: Overlay Crop view
    
    open var overlayView: ImageCropperOverlayView? {
        willSet  {
    ...
    fileprivate func initialize() {
        ...
        // MARK: Add Observers and pass them to foreground image view
    ...
    func layoutSubviews(byImage: Bool) {
        ...
        if byImage {
            ...
            let fitScaleMultiplier = ic_CGSizeFitScaleMultiplier(scrollView.visibleRectangle.size, relativeToSize: reversedFrameWithInsetsSize)
            ...
            if overlayView?.alpha == 1 {
                ...
                size = CGSize(
                    width   : scrollView.visibleRectangle.size.width * fitScaleMultiplier,
                    height  : scrollView.visibleRectangle.size.height * fitScaleMultiplier)
        ...
        overlayView?.cropRect = scrollView.visibleRectangle
    ...
    //  MARK: ImageCropperOverlayViewDelegate
